...
<div class="max-w-6xl mx-auto px-6 py-12">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div>
            ...
            <p class="text-slate-500 mt-2">Gestión de estudiantes del cliente Laravel.</p>
        </div>

        <a href="{{ route('students.create') }}" class="inline-flex items-center justify-center rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white hover:bg-black transition">
            Crear estudiante
        </a>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-6 mb-8">
        ...
        <form method="GET" action="{{ route('students.index') }}" class="flex flex-col md:flex-row gap-4">
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('students.index') }}" class="rounded-xl border border-slate-300 px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition">
                    Limpiar
                </a>
            </div>
        </form>
                ...
                @if ($foundStudent)
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                            <div><span class="font-semibold">ID:</span> {{ $foundStudent['id'] ?? '—' }}</div>
                            <div><span class="font-semibold">Nombre:</span> {{ $foundStudent['name'] ?? '—' }}</div>
                            <div><span class="font-semibold">Email:</span> {{ $foundStudent['email'] ?? '—' }}</div>
                        </div>
                    </div>
                @else
                    <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-700">
                        No se ha encontrado ningún estudiante con ese ID.
                    </div>
                @endif
            </div>
        @endif
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-200">
            <h2 class="text-lg font-semibold text-slate-900">Listado de estudiantes</h2>
        </div>
                    ...
                    <tbody class="divide-y divide-slate-200">
                        @foreach ($students as $student)
                            <tr class="hover:bg-slate-50">
                                <td class="px-6 py-4 text-sm text-slate-700">{{ $student['id'] ?? '—' }}</td>
                                <td class="px-6 py-4 text-sm text-slate-700">{{ $student['name'] ?? '—' }}</td>
                                <td class="px-6 py-4 text-sm text-slate-700">{{ $student['email'] ?? '—' }}</td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <a href="{{ route('students.edit', $student['id']) }}" class="text-sm font-semibold text-slate-900 hover:underline">
                                            Editar
                                        </a>

                                        <form method="POST" action="{{ route('students.destroy', $student['id']) }}" onsubmit="return confirm('¿Seguro que quieres eliminar este estudiante?')">
                                            @csrf
                                            @method('DELETE')
